...
public class CodeEditor extends ViewGroup {
    ...
    public CodeEditor(Context context) throws Exception {
        this(context, null);
    }


    public CodeEditor(Context context, AttributeSet attrs, int defStyle) throws Exception {
        super(context, attrs, defStyle);
        init(context);
    }


    public CodeEditor(Context context, AttributeSet attrs) throws Exception {
	...
	public void init(CodeBean CodeBean){
		bean=CodeBean;
		invalidate();
	}
    /* 初始化一些参数 */
    public void init(Context context)throws CodeBeanException, FileNotFoundException {
		if(CodeBean.Bean==null){
			throw new CodeBeanException("CodeBean is null!!!!!!");
		}
		bean=CodeBean.Bean;
		...
		setBackgroundColor(Color.parseColor(bean.BackGround));
		//Log.e("e",bean.Drawable
		if(!bean.Drawable.equals("null")){
			setBackgroundDrawable(Drawable.createFromStream(new FileInputStream(new File(bean.Drawable)),"code"));

		}
        backPaint = new Paint();
        backPaint.setColor(Color.parseColor(bean.PoinBackRound));
    }
